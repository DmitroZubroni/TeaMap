  ...
  return (
    <span className="inline-flex gap-0.5 align-middle">
      {Array.from({ length: max }).map((_, i) => (
        <span
          key={i}
          className={`w-1.5 h-1.5 rounded-full ${i < value ? 'bg-gold' : 'bg-ink/15'}`}
        />
      ))}
    </span>
  )
...
export default function TeaPanel({ countryId, teaId, onClose, onSelectTea }) {
  ...
  useEffect(() => {
    ...
    setError(null)
    Promise.all([
      getTeaDetail(countryId, teaId),
      getCategories(),
      getSources(),
      getTeaIndex(countryId),
    ])
  ...
  return (
    ...
    >
      ...
      <div className="overflow-y-auto scrollbar-thin">
        {!tea && !error && (
          <div className="p-8 text-center text-ink-soft/60 text-sm">Загружаю карточку…</div>
        )}
        {error && <div className="p-8 text-center text-red-800 text-sm">{error}</div>}

        {tea && (
          <>
            <div className="px-6 pt-7 pb-4" style={{ background: `linear-gradient(180deg, ${color}33, transparent)` }}>
              <span
              ...
              {tea.description?.short && (
                <p className="mt-3 text-[14px] text-ink/90 leading-relaxed italic">
                  «{tea.description.short}»
                </p>
              )}
            </div>
